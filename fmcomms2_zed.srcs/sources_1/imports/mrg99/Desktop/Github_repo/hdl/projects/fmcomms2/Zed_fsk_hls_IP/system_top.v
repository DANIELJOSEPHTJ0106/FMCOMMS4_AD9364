...
//  inout       [ 3:0]      gpio_ctl,           // <-- ??? ??????? ?????????
  ...
  // 49-? ??????? 45 ??? ???? ???????. ?????? ??? gpio_ctl-???? ???????? (43:40) ?????????.
  ad_iobuf #(
    .DATA_WIDTH(45)
  ) i_iobuf_gpio (
    .dio_t ({gpio_t[50:49], gpio_t[46:44], gpio_t[39:0]}),
    .dio_i ({gpio_o[50:49], gpio_o[46:44], gpio_o[39:0]}),
    .dio_o ({gpio_i[50:49], gpio_i[46:44], gpio_i[39:0]}),
...
//            gpio_ctl, // ??????????? ?????????
  ...
  system_wrapper i_system_wrapper (
    ...
    .fmcomms4_ctrl_in_0 (fmcomms4_ctrl_in_0), // <-- Block Design-? ????????? ?????? ????? ?????? ??????
    .gpio_i (gpio_i),
    .gpio_o (gpio_o),
    .gpio_t (gpio_t),
    .hdmi_data (hdmi_data),
